...
if [ $# -eq 1 ]; then # check parameters validity
    id=$1
    mkfifo $id.pipe # create single client pipe
    ...
    while true; do
        ...
        read req db table row # prompt user to insert a command
        case $req in
            create_database)
            ...
            query)
                ...
                if [[ $message == *"Error"* ]]; then
                    ...
                    echo "Start result"
                    echo $message | sed 's/ /\n/g' # message to display table rows on a new lines
                    echo "End result"
                fi
            ;;
            shutdown)
